...
- [x] 변경 할 numberOfGuests의 값이 0 미만ㅌ면 `예외`가 발생한다.
- [x] numberOfGuests 값을 변경할 주문 테이블의 empty 값이 true면 `예외`가 발생한다.

# 단체지정(TableGroup)
## 저장
- [x] 단체지정이 DB에 저장된다.
- [x] 단체지정이 완료된 주문테이블의 empty 값은 false로 변경된다.
- [x] 단체지정이 완료된 주문테이블의 table_group_id 값은 저장된 단체지정의 id값으로 변경된다.
### 제약사항 
- [x] 저장하려는 단체지정의 OrderTable이 null이면 `예외`가 발생한다.
- [x] 단체지정을 하려는 주문테이블의 수가 2 이상이 아니면 `예외`가 발생한다. 
- [x] 단체지정을 하려는 주문테이블들의 id로 찾은 주문테이블의 수와 요청했던 주문테이블의 수가 다르면 `예외`가 발생한다.
  - ex) 단체지정 요청 주문테이블 ids = [1,2], 찾은 주문테이블 [2] 이면 예외 발생
- [x] 단체지정을 하려는 주문테이블의 empty 값이 false이면 `예외`가 발생한다.
- [x] 단체지정을 하려는 주문테이블에 이미 단체지정이 되어있다면 `예외`가 발생한다.
- [x] DB에서 created_date는 null을 허용하지 않는다.
## 수정 - 단체지정 해제
- [x] 단체지정 해제가 완료된 주문테이블의 empty 값은 false로 유지된다.
- [x] 단체지정 해제가 완료된 주문테이블의 table_group_id 값은 null 변경된다.
- [x] 그룹이 해제 됐을 때, tableGroup의 row가 삭제되면 안된다.
### 제약사항
- [x] 해제하려는 테이블의 주문의 주문상태가 COMPLETION이 아니면 `예외`가 발생한다.

# 주문(Order)
## 저장
- [x] 주문이 DB에 저장된다.
- [x] 저장 요청시 OrderStatus는 COOOKING으로 지정된다.
- [x] 저장 요청시 OrderedTime은 현재 시간 으로 지정된다.
### 제약사항
- [x] 저장하려는 주문의 주문항목이 비어잇으면 `예외`가 발생한다.
- [x] 저장하려는 주문의 주문항목들의 menuId로 찾은 메뉴 수와 요청했던 주문항목의 수가 다르면 `예외`가 발생한다.
  - ex) 주문 요청 주문항목의 menuIds = [1,2], 찾은 메뉴 [2] 이면 예외 발생
- [x] 저장하려는 주문의 주문테이블의 존재하지 않으면 `예외`가 발생한다.
- [x] 저장하려는 주문의 주문테이블의 empty 값이 true이면 `예외`가 발생한다.
- [x] DB에서 order_table_id를 가지는 주문테이블이 존재해야한다.
- [x} DB에서 order_table_id는 null을 허용하지 않는다.
- [x] DB에서 order_status는 null을 허용하지 않는다.
- [x] DB에서 ordered_time는 null을 허용하지 않는다.
## 조회 
- [x] DB에 저장된 모든 주문을 반환한다.
  - [ ] DB에 저장된 주문이 없으면 빈 배열을 반환한다.
  - [x] 주문항목도 함께 반환한다.
- [x] 주문 테이블의 id와, 주문상태로 존재 여부를 boolean으로 반환한다.
- [x] 주문 테이블들의 id와, 주문상태로 존재 여부를 boolean으로 반환한다.
## 수정
- [x] 수정 후 반환 시 주문항복이 포함된 주문을 반환한다.
### 제약사항
- [x] 수정하려는 주문이 없다면 `예외`가 발생한다.
- [x] 수정하려는 주문의 주문상태가 COMPLETION이면 `예외`가 발생한다.

# 주문 항목(OrderLineItem)
## 저장 
- [x] 주문항목이 저장된다. 
### 제약사항 
- [x] DB에서 order_id를 가지는 주문이 존재해야한다.
- [x] DB에서 order_id는 null을 허용하지 않는다.
- [x] DB에서 menu_id를 가지는 주문이 존재해야한다.
- [x] DB에서 menu_id는 null을 허용하지 않는다.
## 조회
- [x] id로 DB의 주문항목을 찾아 반환한다.
  - [x] DB에 저장된 주문항목품이 없다면 빈 Optional 객체를 반환한다.
- [x] DB에 저장된 모든 주문항목을 반환한다.
  - [ ] DB에 저장된 주문항목이 없으면 빈 배열을 반환한다.
- [x] orderId로 모든 주문항목를 반환한다.
  - [x] DB에 orderId를 포함하는 주문항목이 없다면 빈 배열을 반환한다.
