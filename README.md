...
### 메뉴 (Menu)
- [x] 메뉴는 1개 이상의 MenuProduct로 구성된다.
- [x] 메뉴 가격은 0원 이상이다.
- [x] 메뉴에 속한 상품 금액의 합은 메뉴의 가격보다 크거나 같아야 한다.
- [x] 메뉴는 특정 메뉴 그룹에 속해야 한다. (카테고리 느낌)
- [x] 메뉴의 메뉴의 목록을 조회할 수 있다.
...
- [x] 주문의 목록을 조회할 수 있다.
- [x] 주문(Order)은 테이블 번호, 주문 상태, 주문 시간, 주문 메뉴 목록을 갖는다.
- [x] 주문 상태는 COOKING(조리), MEAL(식사), COMPLETION(식사 완료)로 나뉘고, 상태를 임의로 바꿀 수 있다.
