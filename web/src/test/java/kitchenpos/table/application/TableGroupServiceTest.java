...
import application.OrderTableChangeEventHandler;
...
import dto.request.OrderTableRequest;
...
import exception.TableGroupException;
...
class TableGroupServiceTest {
    ...
    void 단체_지정하려는_테이블은_2개_이상이어야_한다() {
        ...
        Assertions.assertThatThrownBy(() -> tableGroupService.create(단체_지정_생성_요청))
    ...
    void 이미_지정된_테이블을_단체_지정할_수_없다() {
        ...
        Assertions.assertThatThrownBy(() -> tableGroupService.create(또다른_단체_지정_생성_요청))
                .isInstanceOf(TableGroupException.class)
                .hasMessage("주문 테이블의 상태가 유효하지 않습니다.");
    }
    ...
    void 단체_지정된_주문_테이블의_주문_상태가_조리_또는_식사인_경우_단체_지정을_삭제할_수_없다(OrderStatus orderStatus) {
        ...
        Assertions.assertThatThrownBy(() -> tableGroupService.ungroup(등록된_단체_지정_응답.getId()))
