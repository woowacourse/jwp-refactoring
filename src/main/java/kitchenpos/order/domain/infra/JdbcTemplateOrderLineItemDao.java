...
public class JdbcTemplateOrderLineItemDao implements OrderLineItemDao {
    ...
    public List<OrderLineItem> findAll() {
        String sql = "SELECT seq, order_id, menu_id, menu_name, menu_price, quantity FROM order_line_item";
        return jdbcTemplate.query(sql, (resultSet, rowNumber) -> toEntity(resultSet));
    }

    @Override
    public List<OrderLineItem> findAllByOrderId(Long orderId) {
        String sql = "SELECT seq, order_id, menu_id, menu_name, menu_price, quantity FROM order_line_item WHERE order_id = (:orderId)";
    ...
    private OrderLineItem select(Long id) {
        String sql = "SELECT seq, order_id, menu_id, menu_name, menu_price, quantity FROM order_line_item WHERE seq = (:seq)";
        SqlParameterSource parameters = new MapSqlParameterSource()
                .addValue("seq", id);
