...
class OrderDaoTest extends JdbcDaoTest {
    ...
    void 주문을_아이디로_조회한다() {
        ...
        final Long tableGroupId = 테이블그룹을_저장한다(TABLE_GROUP_NOW.생성()).getId();
        final Long orderTableId = 주문테이블을_저장한다(ORDER_TABLE_NOT_EMPTY_1.생성(tableGroupId)).getId();
        final Order savedOrder = 주문을_저장한다(ORDER_COOKING_1.생성(orderTableId, LocalDateTime.of(2022, 10, 24, 10, 10)));
    ...
    void 특정_주문상태를_갖는_주문테이블이_존재하는지_확인한다() {
        ...
        final Long tableGroupId = 테이블그룹을_저장한다(TABLE_GROUP_NOW.생성()).getId();
        final Long orderTableId = 주문테이블을_저장한다(ORDER_TABLE_NOT_EMPTY_1.생성(tableGroupId)).getId();
        주문을_저장한다(ORDER_COOKING_1.생성(orderTableId, LocalDateTime.of(2022, 10, 24, 10, 10)));
    ...
    void 특정_주문상태를_갖는_주문테이블들이_존재하는지_확인한다() {
        ...
        주문을_저장한다(ORDER_COOKING_1.생성(firstOrderTableId, LocalDateTime.of(2022, 10, 24, 10, 10)));
        final Long secondOrderTableId = 주문테이블을_저장한다(ORDER_TABLE_NOT_EMPTY_1.생성(tableGroupId)).getId();
        주문을_저장한다(ORDER_COOKING_1.생성(secondOrderTableId, LocalDateTime.of(2022, 10, 24, 10, 10)));
