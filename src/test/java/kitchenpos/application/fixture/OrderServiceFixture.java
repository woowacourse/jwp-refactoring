...
public class OrderServiceFixture {
    ...
    @BeforeEach
    void setUp() {
        OrderLineItem 주문_항목1 = new OrderLineItem(1L, 1L, 2);
        OrderLineItem 주문_항목2 = new OrderLineItem(2L, 2L, 2);
        ...
        저장된_주문.setId(1L);
        저장된_주문1 = 저장된_주문;
        저장된_주문2 = new Order(주문_테이블.getId(), 주문_상태, LocalDateTime.now(), 주문_항목들);
        저장된_주문.setId(2L);
        식사_상태의_저장된_주문 = new Order(주문_테이블.getId(), OrderStatus.MEAL.name(), LocalDateTime.now(), 주문_항목들);
        식사_상태의_저장된_주문.setId(저장된_주문.getId());
        계산_상태의_저장된_주문 = new Order(주문_테이블.getId(), OrderStatus.COMPLETION.name(), LocalDateTime.now(), 주문_항목들);
