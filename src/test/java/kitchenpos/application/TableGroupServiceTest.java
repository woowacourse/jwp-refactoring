...
class TableGroupServiceTest extends TableGroupServiceFixture {
    ...
    void 단체_지정을_등록한다() {
        // given
        given(orderTableRepository.findAllByIdIn(anyList())).willReturn(주문_테이블들);
        given(tableGroupDao.save(any(TableGroup.class))).willReturn(저장된_단체_지정);
        given(orderTableRepository.save(any(OrderTable.class))).willReturn(주문_테이블1)
                                                               .willReturn(주문_테이블2);
    ...
    void 단체_지정_등록시_전달된_주문_테이블_개수와_실제_저장되어_있는_주문_테이블_개수가_다르다면_예외를_반환한다() {
        // given
        given(orderTableRepository.findAllByIdIn(anyList())).willReturn(List.of(주문_테이블1));
    ...
    void 단체_지정_등록시_주문_테이블중_비어있지_않은_것이_존재한다면_예외를_반환한다() {
        // given
        given(orderTableRepository.findAllByIdIn(anyList())).willReturn(빈_주문_테이블이_포함된_주문_테이블들);
    ...
    void 단체_지정_등록시_단체_지이_null이_아닌_것이_존재한다면_예외를_반환한다() {
        // given
        given(orderTableRepository.findAllByIdIn(anyList())).willReturn(단체_지정이_null이_아닌_테이블이_포함된_주문_테이블들);
    ...
    void 그룹을_해제한다() {
        // given
        given(orderTableRepository.findAllByTableGroupId(anyLong())).willReturn(주문_테이블들);
