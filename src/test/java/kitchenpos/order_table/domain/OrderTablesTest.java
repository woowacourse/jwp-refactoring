...
class OrderTablesTest {
  ...
  void test_validateNotEmptyOrNotBelongTableGroup_IllegalArgumentException(
    ...
    //when & then
    assertThatThrownBy(() -> orderTables.validateEmptyOrBelongTableGroup())
        .isInstanceOf(IllegalArgumentException.class);
  }

  static Stream orderTable() {
    //주문 테이블이 비어있지 않고, 주문 테이블이 다른 테이블 그룹에 속해있는 경우
    final OrderTable orderTable1 = new OrderTable(1L, 2L, 3, false);

    //주문 테이블이 비어있고, 주문 테이블이 다른 테이블 그룹에 속해있는 경우
    final OrderTable orderTable2 = new OrderTable(1L, 2L, 3, true);

    //주문 테이블이 비어있지 않고, 주문 테이블이 다른 테이블 그룹에 속해있지 않는 경우
    final OrderTable orderTable3 = new OrderTable(1L, 2L, 3, false);
