...
public class MenuGroupAcceptanceTest extends AcceptanceTest {
    ...
    void createMenuGroup() {
        final MenuGroup requestBody = new MenuGroup("사이드");

        final ExtractableResponse<Response> response = 메뉴_그룹_등록_요청(requestBody);

        final MenuGroup responseBody = response.as(MenuGroup.class);
    ...
    void getMenuGroups() {
        // given
        final MenuGroup menuGroup1 = 메뉴_그룹_등록("사이드");
        final MenuGroup menuGroup2 = 메뉴_그룹_등록("와인");

        // when
        final ExtractableResponse<Response> response = 모든_메뉴_그룹_조회_요청();
        final List<MenuGroup> responseBody = response.body()
                .jsonPath()
