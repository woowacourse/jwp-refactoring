...
public class OrdersAcceptanceTest extends AcceptanceTest {
    ...
    void createOrder() {
        ...
        final Order responseBody = response.body().as(Order.class);
        final List<OrderLineItem> orderLineItems = responseBody.getOrderLineItems();
    ...
    void getOrders() {
        final Menu newMenu = 메뉴_등록("파스타세트", 16000L, 세트, 토마토파스타.getId(), 탄산음료.getId());
        final OrderTable table1 = 손님_채운_테이블_생성(2);
        final OrderTable table2 = 손님_채운_테이블_생성(2);
        final Order order1 = 메뉴_주문(table1, 파스타한상.getId());
        final Order order2 = 메뉴_주문(table2, newMenu.getId());

        final ExtractableResponse<Response> response = 모든_주문_조회_요청();
        final List<Order> responseBody = response.body()
                .jsonPath()
                .getList(".", Order.class);

        final List<OrderLineItem> orderLineItems = responseBody.stream()
    ...
    void changeOrderStatus() {
        final Order order = 메뉴_주문(손님있는_테이블, 파스타한상.getId());
        order.changeOrderStatus(OrderStatus.MEAL.name());

        // when
        final ExtractableResponse<Response> response = 주문_상태_변경_요청(order);
        final Order responseBody = response.as(Order.class);
