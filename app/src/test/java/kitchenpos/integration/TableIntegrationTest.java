...
class TableIntegrationTest extends IntegrationTestContext {
    ...
    void 존재하지_않는_테이블에_대해_그룹화하려_하면_예외를_던진다() {
        ...
        // when, then
        Assertions.assertThatThrownBy(() -> tableService.groupOrderTables(tableGroup.getId(), List.of(orderTable.getId(), 999L)))
                .isInstanceOf(OrderTableNotFoundException.class);
    }
}
